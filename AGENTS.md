...
- The commit message should consist of:
  ...
  2. Reference to the feature in the [spec](spec/) folder used to start the agentic run, if any. The spec folder SHOULD ONLY contain the `SPEC.md` part and its immediate parent, i.e. NO `backlog`, `in_progress` and suchlike parts of the spec path (example: WRONG: `Spec: spec/in_progress/feat-k8n4pz-llm-workspace-layout/SPEC.md`, OK: `Spec: feat-k8n4pz-llm-workspace-layout/SPEC.md`)
...
- The rationale for the code can be determined from the commit messages, which usually contain the reference to the spec used for the agentic run, and the summary of the agentic run, both defined in the section `## Agent commit behavior`.
